...
class YoukuNewsreelSpider(scrapy.Spider):
    name = 'Youku_newsreel'
    allowed_domains = ['list.youku.com']
    start_urls = ['http://list.youku.com/category/show/c_84.html']

    def parse(self, response):
        area_page = response.css('a[href^="//list.youku.com/category/show/c_84_s_6_d_1_a_"]')
    ...
    def parse_page_detail_url(self, response):
        ...
        newsreel_urls = response.css('a[href^="//v.youku.com/v_show/id_"]::attr(href)').extract()
        if not newsreel_urls:
            newsreel_urls = response.css('a[href^="//v.youku.com/show/id_"]::attr(href)').extract()
        newsreel_urls = [url for (index, url) in enumerate(newsreel_urls) if index % 2 == 0]
        for newsreel_url in newsreel_urls:
            yield Request(url='https:' + newsreel_url, callback=self.parse_newsreel_info,
    ...
    def parse_newsreel_info(self, response):
        ...
        if detail_url:
            ...
            yield Request(url='https:' + detail_url, callback=self.parse_newsreel_detail,
                          meta={'play_list': play_list, 'name': name, 'area': area, 'type_tag': type_tag, 'parts': parts
                                }, dont_filter=True)

    def parse_newsreel_detail(self, response):
